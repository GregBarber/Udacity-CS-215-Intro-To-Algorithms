...
def mode(L):
    ...
    for num in L:
        if num not in count:
            ...
            if count[num] > maxCount[1] or num == maxCount[0]:
                maxCount = [num, count[num]]
...
def test_actor():
    ...
    for (actor, movie, year) in data: 
        make_link(Graph, actor, movie + ' '+ year.rstrip('\n'))
    ...
    test = ['Jackson, Samuel L.', 'Hoffman, Dustin', 'De Niro, Robert', 'Morrison, Rana']
    for actor in test:
        #actor = actors{test[a]}
       # print(str(a) + ' of ' + str(length))

        cent = centrality(Graph, actor)
        actorCentrality[actor] = cent

    sorted_cent = sorted(actorCentrality.items(), key = operator.itemgetter(1), reverse = True)
    print(sorted_cent[0:20])
...
'''
    ...
    for line in file:
        ...
        if actor not in actors:
            actors[actor] = [movie + ' ' + year.rstrip('\n')]
        else:
            actors[actor].append(movie + ' ' + year.rstrip('\n'))
